...
{
  "statusCode": 400,
  "message": ["property X should not exist", "поле «Y» должно быть..."],
  "error": "Bad Request"
}
```
...
1. **Неизвестные поля (White-listing):** В системе включен `forbidNonWhitelisted: true`. Если вы передаете в Query или Body поля, которые не описаны в DTO на стороне Gateway, запрос будет отклонен.
2. **Типы данных:** Проверьте соответствие типов (UUID, Date, Enum).
3. **Локализация:** Сообщения об ошибках для стандартных валидаторов (UUID, Enum и т.д.) переведены на русский язык.

#### Решения

- **Добавьте поле в DTO:** Если поле необходимо, добавьте его в соответствующий DTO в библиотеке `libs/contracts/src/gateway/...`.
- **Проверьте @ApiQuery:** Если поле описано в Swagger через `@ApiQuery`, оно **обязательно** должно быть и в классе DTO, который используется в `@Query()`.
- **Используйте ParseUUIDPipe:** Для параметров пути в контроллерах используйте `ParseUUIDPipe` с кастомным `exceptionFactory` для вывода русских сообщений.
