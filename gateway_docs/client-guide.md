...
- Тело: `{ email, password, fullName, role? }`
...
- Аутентификация: refresh token из httpOnly cookie `refreshToken`
- CSRF: требуется для браузера (заголовок `X-CSRF-Token`)
...
| Локации | `GET/POST /personnel/locations`, `PATCH /personnel/locations/:id` |
| Производственные линии | `POST /personnel/production-lines` |
| Рабочие места | `POST /personnel/workstations`, `POST /personnel/workstations/assign` |
