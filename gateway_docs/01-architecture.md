...
- **PostgreSQL (Prisma)**
  ...
  - Personnel Service: Department, Position, Employee, Location, Workstation, ShiftScheduleTemplate
  - Production Service: Product, ProductionOrder, Sale, Inventory, QualityResult, QualitySpec, Sensor, SensorParameter, Customer, Warehouse
...
Система приведена к третьей нормальной форме (3NF) для устранения транзитивных зависимостей:

#### Personnel Service
- `Location` — отдельная сущность для географических данных
- `Workstation` — рабочие места с привязкой к локации и производственной линии
- `Employee.locationId` удален → локация определяется через `workstation.locationId` или `employee.department.locationId`
- `Position.departmentId` удален (v1.2.0 refactor) → связь Position → Department теперь идет через Employee

#### Production Service
- `Customer` — отдельная сущность для клиентов (вынесена из Sale)
- `SensorParameter` — параметры датчиков (температура, давление и т.д.)
- `Sensor` — связь: ProductionLine + SensorParameter
- `Warehouse` — отдельная сущность для складов (вынесена из Inventory)
- `Inventory` — связь: Product + Warehouse (вместо warehouseCode)
- `QualitySpec` — отдельная сущность для спецификаций качества

Все новые сущности содержат необходимые поля для интеграции и имеют индексы для производительности.
