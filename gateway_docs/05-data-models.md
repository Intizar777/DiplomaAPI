...
## Версия 1.2.0: 3NF Нормализация

**Дата обновления:** май 2026  
**Цель:** Устранение транзитивных зависимостей и приведение схемы к третьей нормальной форме (3NF)

### Основные изменения

#### Personnel Service
- ✅ `Location` — отдельная сущность для всех географических данных
- ✅ `Workstation` — рабочие места (без привязки к производственным линиям в personnel)
- ✅ `Employee.locationId` → удален (локация определяется через workstation или department)
- ⚠️ `ProductionLine` — теперь только в Production Service (не дублируется)

#### Production Service
- ✅ `Customer` — отдельная сущность клиентов (ранее был встроен в Sale)
- ✅ `SensorParameter` — справочник параметров датчиков (Температура, Давление, и т.д.)
- ✅ `Sensor` — датчик связан с параметром и производственной линией
- ✅ `SensorReading` — показание датчика (вместо встроенного parameterName)
- ✅ `Warehouse` — справочник складов (ранее был кодом в Inventory)
- ✅ `Inventory` — связь Product + Warehouse (вместо warehouseCode)
- ✅ `QualitySpec` — спецификации качества (отдельно от QualityResult)

### Миграционные заметки

Если вы работаете с данными, загруженными до версии 1.2.0:
1. Используйте seed-скрипты для пересоздания БД: `npm run seed:all`
2. Запустите все миграции: `npx prisma migrate deploy`
3. Генерируйте Prisma клиент: `npx prisma generate`
...
Модель почтового индекса (3NF нормализация).
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `postalCode` | String @unique | Почтовый индекс | `@db.VarChar(20)` |
| `city` | String | Город | `@db.VarChar(100)` |
| `region` | String | Регион | `@db.VarChar(100)` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |

#### Связи

- `locations` — один ко многим с Location

#### Индексы

- `postalCode` — уникальный индекс
...
model PostalArea {
  id         String   @id @default(uuid()) @db.Uuid
  postalCode String   @unique @map("postal_code") @db.VarChar(20)
...
Модель локации (офис, завод, склад).
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `name` | String | Название локации | `@db.VarChar(200)` |
| `code` | String @unique | Код локации | `@db.VarChar(20)` |
| `type` | LocationType | Тип локации | |
| `streetAddress` | String | Адрес улицы | `@map("street_address") @db.VarChar(255)` |
| `postalAreaId` | UUID? | ID почтового индекса (опционально) | `@map("postal_area_id") @db.Uuid` |
| `sourceSystemId` | String? @unique | ID в исходной системе | `@map("source_system_id") @db.VarChar(20)` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |
| `updatedAt` | DateTime | Дата обновления | `@updatedAt @map("updated_at")` |
...
- `postalArea` — многие к одному с PostalArea (опционально)
- `departments` — один ко многим с Department
...
В версии 1.2.0 произведена нормализация: поля `country`, `region`, `city`, `postalCode` вынесены в отдельную сущность `PostalArea` для соблюдения 3NF. Локация теперь содержит только адресную часть и ссылку на почтовый индекс.
...
model Location {
  ...
  postalAreaId   String?      @map("postal_area_id") @db.Uuid
  sourceSystemId String?      @unique @map("source_system_id") @db.VarChar(20)
  createdAt      DateTime     @default(now()) @map("created_at")
  updatedAt      DateTime     @updatedAt @map("updated_at")

  postalArea    PostalArea? @relation(fields: [postalAreaId], references: [id], onDelete: SetNull)
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `name` | String | Название рабочего места | `@db.VarChar(100)` |
| `code` | String | Код рабочего места | `@db.VarChar(20)` |
| `locationId` | UUID | ID локации | `@map("location_id") @db.Uuid` |
| `productionLineId` | UUID? | ID производственной линии | `@map("production_line_id") @db.Uuid` |
| `workstationType` | WorkstationType | Тип рабочего места | `@map("workstation_type")` |
| `sourceSystemId` | String? @unique | ID в исходной системе | `@map("source_system_id") @db.VarChar(20)` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |
| `updatedAt` | DateTime | Дата обновления | `@updatedAt @map("updated_at")` |
...
- `location` — многие к одному с Location
- `productionLine` — многие к одному с ProductionLine
- `employees` — один ко многим с Employee
...
Модель связи подразделения и производственной линии (many-to-many).

**Таблица:** `department_production_lines`  
**Схема:** `apps/personnel/prisma/schema.prisma`
...
- Уникальный индекс на `(departmentId, productionLineId)`
- `departmentId` — индекс
- `productionLineId` — индекс
...
```prisma
model DepartmentProductionLine {
  id               String   @id @default(uuid()) @db.Uuid
  departmentId     String   @map("department_id") @db.Uuid
  productionLineId String   @map("production_line_id") @db.Uuid
  createdAt        DateTime @default(now()) @map("created_at")
  ...
  @@unique([departmentId, productionLineId])
  @@index([departmentId])
  @@index([productionLineId])
  @@map("department_production_lines")
}
```
...
**Таблица:** `workstation_type`

| Значение | База данных | Описание |
|----------|-------------|----------|
| `OPERATOR_POST` | `operator_post` | Операторский пост |
| `CONTROL_POINT` | `control_point` | Контрольная точка |
| `LOADING_AREA` | `loading_area` | Зона погрузки |
| `LAB_STATION` | `lab_station` | Лабораторная станция |
| `OFFICE_DESK` | `office_desk` | Офисный стол |
...
- `qualitySpecs` — один ко многим с QualitySpec
...
- `code` — индекс
- `sourceSystemId` — уникальный индекс
- `category, brand` — составной индекс
- `brand` — индекс
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `name` | String | Название клиента | `@db.VarChar(200)` |
| `code` | String @unique | Код клиента | `@db.VarChar(20)` |
| `region` | String | Регион | `@db.VarChar(100)` |
| `sourceSystemId` | String? @unique | ID в исходной системе | `@map("source_system_id") @db.VarChar(20)` |
| `isActive` | Boolean | Активен | `@default(true) @map("is_active")` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |
| `updatedAt` | DateTime | Дата обновления | `@updatedAt @map("updated_at")` |
...
| `productId` | UUID | ID продукта | `@map("product_id") @db.Uuid` |
| `customerId` | UUID? | ID клиента (опционально) | `@map("customer_id") @db.Uuid` |
| `quantity` | Decimal | Количество | `@db.Decimal(15, 3)` |
...
- `product` — многие к одному с Product
- `customer` — многие к одному с Customer (опционально)
...
Модель склада (3NF нормализация).
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `name` | String | Название склада | `@db.VarChar(150)` |
| `code` | String @unique | Код склада | `@db.VarChar(20)` |
| `location` | String | Локация склада | `@db.VarChar(200)` |
| `capacity` | Decimal? | Вместимость (м³ или единиц) | `@db.Decimal(15, 2)` |
| `isActive` | Boolean | Активен | `@default(true) @map("is_active")` |
| `sourceSystemId` | String? @unique | ID в исходной системе | `@map("source_system_id") @db.VarChar(20)` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |
| `updatedAt` | DateTime | Дата обновления | `@updatedAt @map("updated_at")` |
...
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `productId` | UUID | ID продукта | `@map("product_id") @db.Uuid` |
| `warehouseId` | UUID | ID склада | `@map("warehouse_id") @db.Uuid` |
| `lotNumber` | String? | Номер партии | `@map("lot_number") @db.VarChar(20)` |
| `quantity` | Decimal | Количество | `@db.Decimal(15, 3)` |
| `unitOfMeasure` | String | Единица измерения | `@map("unit_of_measure") @db.VarChar(10)` |
...
- `product` — многие к одному с Product
- `warehouse` — многие к одному с Warehouse

#### Индексы

- `productId` — индекс
- `warehouseId` — индекс
- `productId, warehouseId` — составной индекс
...
| `qualitySpecId` | UUID? | ID спецификации качества | `@map("quality_spec_id") @db.Uuid` |
| `qualityStatus` | QualityStatus | Статус качества | `@map("quality_status")` |
...
- `product` — многие к одному с Product
- `qualitySpec` — многие к одному с QualitySpec

#### Индексы

- `lotNumber` — индекс
- `productId` — индекс
- `qualityStatus` — индекс
- `testDate` — индекс
- `productId, testDate` — составной индекс
...
model QualityResult {
  ...
  qualitySpecId String?         @map("quality_spec_id") @db.Uuid
  qualityStatus QualityStatus   @map("quality_status")
  testDate      DateTime        @map("test_date")
  createdAt     DateTime        @default(now()) @map("created_at")

  product     Product     @relation(fields: [productId], references: [id])
  qualitySpec QualitySpec? @relation(fields: [qualitySpecId], references: [id])
...
**Примечание:** Спецификации качества (lowerLimit, upperLimit) вынесены в отдельную сущность `QualitySpec` для соблюдения 3NF.
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `sensorId` | UUID | ID датчика | `@map("sensor_id") @db.Uuid` |
...
- `sensor` — многие к одному с Sensor

#### Индексы

- `sensorId` — индекс
- `recordedAt` — индекс
- `recordedAt, quality` — составной индекс
...
model SensorReading {
  ...
  @@index([sensorId])
  @@index([recordedAt])
  @@index([recordedAt, quality])
  @@map("sensor_readings")
}
```

**Примечание:** Метаданные датчика (deviceId, productionLine, parameterName, unit) вынесены в отдельную сущность `Sensor` для соблюдения 3NF.

---

### SensorParameter

Модель параметра датчика (3NF нормализация).
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `name` | String | Название параметра (на русском) | `@db.VarChar(100)` |
| `code` | String @unique | Код параметра | `@db.VarChar(20)` |
...
- `sensors` — один ко многим с Sensor
...
|------|-----|----------|-------------|
| `id` | UUID @id | Уникальный идентификатор | `@default(uuid())` |
| `deviceId` | String | ID устройства (уникальный) | `@unique @map("device_id") @db.VarChar(50)` |
| `productionLineId` | UUID | ID производственной линии | `@map("production_line_id") @db.Uuid` |
| `sensorParameterId` | UUID | ID параметра датчика | `@map("sensor_parameter_id") @db.Uuid` |
| `isActive` | Boolean | Активен | `@default(true) @map("is_active")` |
| `createdAt` | DateTime | Дата создания | `@default(now()) @map("created_at")` |
| `updatedAt` | DateTime | Дата обновления | `@updatedAt @map("updated_at")` |
...
- `productionLine` — многие к одному с ProductionLine
- `sensorParameter` — многие к одному с SensorParameter
- `readings` — один ко многим с SensorReading

#### Индексы

- `productionLineId` — индекс
- `sensorParameterId` — индекс
- `deviceId` — уникальный индекс
...
model Sensor {
  ...
  productionLineId  String   @map("production_line_id") @db.Uuid
  sensorParameterId String   @map("sensor_parameter_id") @db.Uuid
  isActive          Boolean  @default(true) @map("is_active")
  createdAt         DateTime @default(now()) @map("created_at")
  updatedAt         DateTime @updatedAt @map("updated_at")

  productionLine  ProductionLine  @relation(fields: [productionLineId], references: [id])
  sensorParameter SensorParameter @relation(fields: [sensorParameterId], references: [id])
  readings        SensorReading[]

  @@index([productionLineId])
  @@index([sensorParameterId])
  @@map("sensors")
}
```
...
- `productId` — индекс
- Уникальный индекс на `(productId, parameterName)`
...
model QualitySpec {
  ...
  @@unique([productId, parameterName])
  @@index([productId])
  @@map("quality_specs")
}
```

**Примечание:** Вынесена для соблюдения 3NF - спецификации качества (lowerLimit, upperLimit) теперь хранятся отдельно от конкретных результатов тестов.
