...
@pytest.mark.asyncio
async def test_get_marginality_returns_correct_structure(kpi_service: ProductionAnalyticsService, sample_cost_base):
    """Test marginality returns correct structure."""
    result = await kpi_service.get_marginality(
        from_date=date(2026, 5, 1),
        to_date=date(2026, 5, 1),
    )
    ...
    assert "target" in result
    assert result["target"] == Decimal("18")
    assert "status" in result


@pytest.mark.asyncio
async def test_get_cost_per_kg_returns_correct_structure(kpi_service: ProductionAnalyticsService, sample_cost_base):
    """Test cost per kg returns correct structure."""
    result = await kpi_service.get_cost_per_kg(
        from_date=date(2026, 5, 1),
        to_date=date(2026, 5, 1),
    )
...
@pytest.mark.asyncio
async def test_get_ebitda_returns_correct_structure(kpi_service: ProductionAnalyticsService, sample_cost_base):
    """Test EBITDA returns correct structure."""
    result = await kpi_service.get_ebitda(
        from_date=date(2026, 5, 1),
        to_date=date(2026, 5, 1),
    )

    assert "ebitda_million_rub" in result
    assert "ebitda_value" in result
    assert "total_revenue" in result
...
@pytest.mark.asyncio
async def test_get_market_share_returns_correct_structure(kpi_service: ProductionAnalyticsService):
    """Test market share returns correct structure."""
    result = await kpi_service.get_market_share(
        from_date=date(2026, 5, 1),
        to_date=date(2026, 5, 1),
    )

    assert "market_share_percent" in result
    assert "company_volume_tonnes" in result
    assert "market_total_tonnes" in result
    assert "period" in result
