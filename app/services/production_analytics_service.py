...
from app.models import AggregatedKPI, OrderSnapshot, QualityResult, SaleRecord, ProductionLine, BatchInput, ProductionOutput, DowntimeEvent, KPIConfig, CostBase
import structlog
...
class ProductionAnalyticsService:
    ...
    async def get_marginality(
        ...
        """Get marginality (profit margin) using CostBase data."""
        logger.info("get_marginality", from_date=from_date, to_date=to_date)
        ...
        if product_id:
            revenue_query = revenue_query.where(SaleRecord.product_id == product_id)

        revenue_result = await self.db.execute(revenue_query)
        total_revenue = revenue_result.scalar() or Decimal("0")

        # Get material costs from BatchInput + CostBase
        batch_query = select(
            func.sum(BatchInput.quantity).label("total_batch_qty"),
        ).where(
            and_(
                BatchInput.input_date >= datetime.combine(from_date, datetime.min.time()),
                BatchInput.input_date <= datetime.combine(to_date, datetime.max.time()),
            )
        )

        if product_id:
            batch_query = batch_query.where(BatchInput.product_id == product_id)

        batch_result = await self.db.execute(batch_query)
        total_batch_qty = batch_result.scalar() or Decimal("0")

        # Get average raw material cost
        cost_query = select(
            func.avg(CostBase.raw_material_cost).label("avg_material_cost"),
        ).where(
            and_(
                CostBase.period_from <= to_date,
            )
        )

        if product_id:
            cost_query = cost_query.where(CostBase.product_id == product_id)
        else:
            cost_query = cost_query.where(CostBase.product_id.is_(None))

        cost_result = await self.db.execute(cost_query)
        avg_material_cost = cost_result.scalar() or Decimal("0")

        material_cost = total_batch_qty * avg_material_cost
        margin = total_revenue - material_cost
        margin_percent = (margin / total_revenue * 100) if total_revenue > 0 else Decimal("0")
        target = Decimal("18")
        ...
        return {
            ...
            "margin": margin,
            "target": target,
            "status": self._get_status(margin_percent, target, target - Decimal("5"), target + Decimal("10")),
            "period": {"from": from_date.isoformat(), "to": to_date.isoformat()},
        }
    ...
    async def get_cost_per_kg(
        ...
        logger.info("get_cost_per_kg", from_date=from_date, to_date=to_date)

        # Get total production output
        output_query = select(
            func.sum(ProductionOutput.quantity).label("total_output"),
        ).where(
            and_(
        ...
        if product_id:
            output_query = output_query.where(ProductionOutput.product_id == product_id)

        output_result = await self.db.execute(output_query)
        total_output = output_result.scalar() or Decimal("0")
        ...
        total_cost = total_batch_qty * avg_material_cost
        cost_per_kg = (total_cost / total_output) if total_output > 0 else Decimal("0")
    ...
    async def get_ebitda(
        ...
        """Get EBITDA (simplified: Revenue - OperatingCosts)."""
        logger.info("get_ebitda", from_date=from_date, to_date=to_date)
        ...
        # Operating costs (material only for simplicity)
        batch_query = select(
            func.sum(BatchInput.quantity).label("total_batch_qty"),
        ).where(
            and_(
        ...
        cost_query = select(
            ...
            and_(
                CostBase.period_from <= to_date,
                CostBase.product_id.is_(None),  # Global default
            )
        )
        ...
        material_cost = total_batch_qty * avg_material_cost
        ebitda_value = total_revenue - material_cost
        ebitda_million_rub = ebitda_value / Decimal("1000000")

        # Get target from KPIConfig
        target_query = select(KPIConfig.value).where(KPIConfig.key == "ebitda_target_million_rub")
        target_result = await self.db.execute(target_query)
        target = target_result.scalar() or Decimal("0")

        ebitda_margin_percent = (ebitda_value / total_revenue * 100) if total_revenue > 0 else Decimal("0")

        return {
            "ebitda_million_rub": ebitda_million_rub,
            "ebitda_value": ebitda_value,
            "total_revenue": total_revenue,
            "operating_costs": material_cost,
            "ebitda_margin_percent": ebitda_margin_percent,
            "target_million_rub": target,
            "period": {"from": from_date.isoformat(), "to": to_date.isoformat()},
        }
    ...
    async def get_market_share(
        ...
        logger.info("get_market_share", from_date=from_date, to_date=to_date)

        # Company sales volume (in tonnes)
        sales_query = select(
            func.sum(SaleRecord.quantity).label("company_volume"),
        ).where(
            and_(
                SaleRecord.sale_date >= from_date,
                SaleRecord.sale_date <= to_date,
        ...
        company_volume = sales_result.scalar() or Decimal("0")

        # Get market total volume from KPIConfig
        market_query = select(KPIConfig.value).where(KPIConfig.key == "market_total_volume_tonnes")
        market_result = await self.db.execute(market_query)
        market_total = market_result.scalar() or Decimal("0")

        market_share_percent = (company_volume / market_total * 100) if market_total > 0 else Decimal("0")

        return {
            "market_share_percent": market_share_percent,
            "company_volume_tonnes": company_volume,
            "market_total_tonnes": market_total,
            "period": {"from": from_date.isoformat(), "to": to_date.isoformat()},
        }
